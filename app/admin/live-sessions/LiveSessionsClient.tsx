      ...
      <div>
        ...
        <form action={formAction} className="rounded-2xl border border-navy/10 bg-white p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="zoom_url" className="text-[11px] font-bold tracking-[0.06em] uppercase text-navy/55">Zoom Registration URL</label>
              <input id="zoom_url" name="zoom_url" type="url" placeholder="https://us02web.zoom.us/meeting/register/..." required className="field" />
            </div>
          </div>
...
function SessionList({ sessions }: { sessions: Session[] }) {
  if (sessions.length === 0) {
    return <p className="text-[13px] text-navy/40">None scheduled.</p>;
  }
  return (
      ...
      {sessions.map((s) => (
        <div key={s.id} className="rounded-xl border border-navy/10 bg-white px-5 py-4 flex items-center justify-between gap-4">
          ...
          <form action={deleteLiveSessionAction}>
            ...
            <button type="submit" className="text-[12px] font-semibold text-red-500 hover:text-red-700 transition cursor-pointer">Delete</button>
          </form>
        </div>
      ))}
    </div>
